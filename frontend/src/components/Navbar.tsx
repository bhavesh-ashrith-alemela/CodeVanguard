    ...
    <div className="sticky top-0 z-50 w-full bg-white border-b-4 border-slate-900">
      <div className="navbar max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between py-2">
        ...
        <div className="flex-none gap-2">
          ...
          <div className="dropdown dropdown-end md:hidden relative inline-block">
            <div tabIndex={0} role="button" className="btn btn-ghost btn-circle border-2 border-slate-900 bg-white shadow-[2px_2px_0px_#0f172a] hover:bg-slate-50 flex items-center justify-center w-10 h-10 rounded-full cursor-pointer">
              <Menu className="w-5 h-5" />
            </div>
            <ul tabIndex={0} className="dropdown-content absolute right-0 mt-3 z-[1] p-2 shadow-[4px_4px_0px_#0f172a] bg-white border-4 border-slate-900 rounded-xl w-52 text-slate-800 font-extrabold text-xs list-none">
              <li className="my-1"><Link href="/" className="block p-2 hover:bg-slate-100 rounded-md"><PlusCircle className="w-4 h-4 inline mr-2" />Scan</Link></li>
              <li className="my-1"><Link href="/examples" className="block p-2 hover:bg-slate-100 rounded-md"><Code className="w-4 h-4 inline mr-2" />Examples</Link></li>
              <li className="my-1"><Link href="/history" className="block p-2 hover:bg-slate-100 rounded-md"><History className="w-4 h-4 inline mr-2" />History</Link></li>
              <div className="divider my-1 border-t-2 border-slate-200"></div>
              {adminUsername ? (
                <>
                  <li className="my-1"><Link href="/admin/dashboard" className="block p-2 text-primary hover:bg-slate-100 rounded-md"><Shield className="w-4 h-4 inline mr-2" />Dashboard</Link></li>
                  <li className="my-1"><button onClick={handleLogout} className="block w-full text-left p-2 text-red-500 hover:bg-slate-100 rounded-md font-extrabold cursor-pointer"><LogOut className="w-4 h-4 inline mr-2" />Logout</button></li>
                </>
              ) : (
                <li className="my-1"><Link href="/admin/login" className="block p-2 hover:bg-slate-100 rounded-md"><Lock className="w-4 h-4 inline mr-2" />Admin Portal</Link></li>
              )}
            </ul>
          </div>
        </div>
